-- 0047 — Follow-up só com lead JÁ TRABALHADO (regra de Roberto, 24/09/2026).
-- Lead sem nenhuma atividade registrada não é follow-up: sai da Esteira, de Sem próximo passo e de todos os contadores
-- (inclusive lead quente). Atrasado = tem atividade + data de próximo contato anterior a hoje.
-- "Sem próximo passo" passa a mostrar só lead com atividade e sem data (trabalhado e esquecido).
-- Sobre a 0046 (mesma assinatura; grants e p_cidade preservados). APLICADA EM PRODUÇÃO via MCP.
-- ROLLBACK: reaplicar 0046_followup_atrasados_sem_quente_fora_do_filtro.sql.
